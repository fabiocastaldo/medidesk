...
import { getAuthHeaders } from "./core/aws-auth.mjs";
...
export class AnthropicBedrockMantle extends BaseAnthropic {
     ...
     */
    constructor({ awsRegion, baseURL, apiKey, awsAccessKey = null, awsSecretAccessKey = null, awsSessionToken = null, awsProfile, providerChainResolver = null, skipAuth = false, ...opts } = {}) {
        ...
        if (!resolvedBaseURL) {
            throw new Errors.AnthropicError('No AWS region or base URL found. Set `awsRegion` in the constructor, the `AWS_REGION` / `AWS_DEFAULT_REGION` environment variable, or provide a `baseURL` / `ANTHROPIC_BEDROCK_MANTLE_BASE_URL` environment variable.');
        }
        ...
        this.beta = makeMantleBetaResource(this);
        this.skipAuth = false;
...
function makeMantleBetaResource(client) {
    const { messages } = new Resources.Beta(client);
    return { messages };
}
//# sourceMappingURL=mantle-client.mjs.map