...
export default async function handler(req, res) {
  ...
  const medicoRes = await fetch(
    `${supabaseUrl}/rest/v1/medici?user_id=eq.${encodeURIComponent(userData.id)}&select=id,stato,specializzazione,piano,created_at,moduli`,
  ...
  const quota = await checkSupabaseRateLimit(quotaKey, 'ottimizza-comunicazione', RATE_LIMIT, 3600);
  if (quota === null) {
