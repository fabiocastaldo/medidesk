...
export default async function handler(req, res) {
  ...
  const medicoRes = await fetch(
    `${supabaseUrl}/rest/v1/medici?user_id=eq.${encodeURIComponent(userData.id)}&select=id,stato,piano,created_at`,
  ...
  const quota = await checkSupabaseRateLimit(quotaKey, 'genera-presentazione', RATE_LIMIT, 3600);
  if (quota === null) {
