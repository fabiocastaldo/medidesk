...
export default async function handler(req, res) {
  ...
  const segRes = await fetch(
    `${supabaseUrl}/rest/v1/segreterie?user_id=eq.${encodeURIComponent(userData.id)}&select=stato,ruolo,cooperativa_id,cooperative(mfa_obbligatoria,id,stato,booking_pubblico)`,
    { headers: srvHeaders }
  ).catch(() => null);
