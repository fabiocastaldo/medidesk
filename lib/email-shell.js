...
export function detailCard(rowsHtml) {
  // Larghezze in PIXEL, non width:100%: Gmail mobile riduce le tabelle annidate
  // con width:100% alla larghezza del contenuto (la tabella root regge perche' ha
  // width="560" come attributo HTML). 480 = 560 (card) - 2*40 (padding del td body);
  // 432 = 480 - 2*24 (padding della cella interna).
  return `<table width="480" cellpadding="0" cellspacing="0" role="presentation" style="width:480px;background:${C.detailBg};border:1px solid ${C.detailBorder};border-radius:8px;margin-bottom:28px;">
  <tr><td style="padding:20px 24px;">
    <table width="432" cellpadding="0" cellspacing="0" role="presentation" style="width:432px;table-layout:fixed;">${rowsHtml}</table>
  </td></tr>
</table>`;
}
...
export function detailRow(label, value, { last = false } = {}) {
  ...
  return `<tr><td style="padding:8px 0;${sep}">
