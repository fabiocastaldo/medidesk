...
export function detailCard(rowsHtml) {
  // L'app Gmail iOS NON onora width:100% sulle tabelle (le riduce al contenuto):
  // larghezza FISSA in pixel = 480, uguale al wrapper del corpo in emailShell.
  // (Browser e iOS Mail riempivano gia'; il noteBox sembra pieno solo perche' il
  // suo testo lungo riempie da se'.)
  return `<table width="480" cellpadding="0" cellspacing="0" style="width:480px;max-width:480px;background:${C.detailBg};border:1px solid ${C.detailBorder};border-radius:8px;margin-bottom:28px;">${rowsHtml}</table>`;
}
