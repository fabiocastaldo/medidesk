...
export function detailCard(rowsHtml) {
  // Niente tabella: l'app Gmail iOS dimensiona le TABELLE al contenuto (anche con width
  // fissa in px), mentre riempie i blocchi <div>/<p> come i paragrafi del corpo. Quindi
  // card = <div> a blocco, righe = <div> (detailRow). Outlook: angoli quadrati (no radius), ok.
  return `<div style="background:${C.detailBg};border:1px solid ${C.detailBorder};border-radius:8px;margin-bottom:28px;padding:12px 24px;">${rowsHtml}</div>`;
}
