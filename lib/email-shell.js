...
export function emailShell(bodyHtml, { footerNote } = {}) {
  const foot = footerNote || 'Messaggio automatico da Delphi&tilde;Med &middot; Non rispondere a questa email';
  return `<!DOCTYPE html>
<html lang="it">
...
export function detailCard(rowsHtml) {
  return `<table width="100%" cellpadding="0" cellspacing="0" style="background:${C.detailBg};border:1px solid ${C.detailBorder};border-radius:8px;margin:0 0 28px;">
...
export function detailRow(label, value, { last = false, iconUrl } = {}) {
  const sep = last ? '' : `border-bottom:1px solid ${C.detailSep};`;
  const ic = iconUrl
    ? `<img src="${iconUrl}" width="14" height="14" alt="" style="vertical-align:-2px;margin-right:6px;border:0;">`
    : '';
  return `<tr><td style="padding:9px 0;${sep}">
    <span style="color:#6a6a6a;font-size:11px;text-transform:uppercase;letter-spacing:.6px;">${ic}${label}</span><br>
...
export function noteBox(html, { tone = 'warn' } = {}) {
  ...
  return `<table width="100%" cellpadding="0" cellspacing="0" style="background:${bg};border-left:3px solid ${bar};border-radius:0 6px 6px 0;margin:0 0 28px;">
