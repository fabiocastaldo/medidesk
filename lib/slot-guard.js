...
// oppure da una giornata singola (disponibilita_singole) sulla stessa data, il
// medico non deve essere in chiusura su quella data per quel centro, e la giornata
// non deve essere gia' occupata dall'agenda del centro (blocchi_occupazione).
...
export async function verificaSlot({ sb, medicoId, centroId, data, ora }) {
  ...
  let turni, singole, chiusure, blocchi;
  try {
    const [tR, sR, cR, bR] = await Promise.all([
      ...
      sb(`blocchi_occupazione?medico_id=eq.${enc(medicoId)}&centro_id=eq.${enc(centroId)}&data=eq.${enc(data)}&select=id`)
    ]);
    if (!tR.ok || !sR.ok || !cR.ok || !bR.ok) return { ok: false, status: 502, error: 'Verifica disponibilita non riuscita' };
    turni = await tR.json(); singole = await sR.json(); chiusure = await cR.json(); blocchi = await bR.json();
    if (!Array.isArray(turni) || !Array.isArray(singole) || !Array.isArray(chiusure) || !Array.isArray(blocchi)) {
  ...
  if (blocchi.length > 0) return { ok: false, status: 422, error: 'Giornata gi\u00e0 occupata dall\u2019agenda del centro' };
