...
function firma(secret, body, codice) {
  return createHmac('sha256', secret).update(`${SCOPO}|${body}|${codice}`, 'utf8').digest();
}

export function creaSfida(secret, email) {
  ...
  return { codice, sfida: body + '.' + firma(secret, body, codice).toString('base64url') };
}
...
export function verificaCodice(secret, sfida, codice, email) {
  ...
  const expected = firma(secret, parts[0], c);
